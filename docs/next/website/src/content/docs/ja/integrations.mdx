---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、Qwen Code、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Qwen Code、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
このフックは、Qwen Code のセッション、プロンプト、ツール、許可、通知、コンパクション、停止、解放の各イベントを、権威ある `working`、`blocked`、`idle` 状態へマッピングします。Qwen が Herdr ペイン内で動作しフックが有効な間は、ライフサイクル状態はフックから得られます。フックがない場合は Qwen のスクリーンマニフェストへフォールバックします。

Herdr はデフォルトで `~/.qwen` を使い、`QWEN_HOME` が設定されていればそちらを使います。Qwen の設定ディレクトリはあらかじめ存在している必要があります。インストールは `hooks/herdr-agent-state.sh`（Windows では `.ps1`）を書き込み、`settings.json` のトップレベル `hooks` オブジェクトに Herdr 所有のエントリだけを追加します。既存のモデル、MCP サーバー、設定、サードパーティーフックは保持されます。再インストールは重複させずに Herdr エントリを修復し、アンインストールは Herdr エントリとスクリプトだけを削除します。

フックは Qwen のセッション id をライブペインの識別情報とネイティブ復元のために記録します。Herdr は `qwen --resume <id>` で保存済みの正確な会話を resume します。既存のバージョン `1` フックは必要な id をすでに報告するため、現行のインテグレーションを再インストールする必要はありません。
