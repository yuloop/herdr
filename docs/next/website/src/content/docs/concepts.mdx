...
Right-click an existing pane to preview and apply a new arrangement without restarting any pane process. **Reposition pane** moves the clicked pane beside a target at a 50:50 split; hover or click a target edge, use `Tab` to cycle targets, and use arrows or `h/j/k/l` to choose an edge. **Layout templates** offers columns, rows, balanced grid, main-left, and main-top; the clicked pane becomes the first or main pane and the others keep their current visual order. Press `Enter` to apply or `Esc` to cancel. These actions affect only the current tab and are unavailable while it is zoomed.
