...
| 弹窗 | `popup.close` |
| 布局 | `layout.export`、`layout.apply`、`layout.set_split_ratio`、`layout.rearrange` |
...
`layout.rearrange` 只重建已有运行窗格周围的布局树，保留窗格 id、终端归属、进程状态和每个窗格的回滚缓冲。重新放置操作把 `source_pane_id` 移到 `target_pane_id` 旁边；`placement` 可为 `left`、`right`、`up` 或 `down`，可选的 `ratio` 默认为 `0.5`。模板操作会让 `anchor_pane_id` 成为第一个或主窗格，其余窗格保持当前视觉顺序。模板可为 `columns`、`rows`、`grid`、`main_left` 和 `main_top`。
...
源窗格和目标窗格必须属于同一个未缩放标签页。响应是 `type: "layout_rearrange"`，`rearrange` 下包含 `changed`、可选的 `reason`、锚点/目标/聚焦窗格 id 和更新后的布局快照。未改变时的 reason 可为 `same_pane`、`cross_tab`、`zoomed_tab`、`single_pane` 或 `unchanged`。成功修改后会发出 `layout.updated`。
