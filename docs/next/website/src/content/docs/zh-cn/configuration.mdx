...
未设置或值为空时，Herdr 在 Unix 上依次使用 `$SHELL`、`/bin/sh`；在 Windows 上则依次尝试 PowerShell 7（`pwsh.exe`）、Windows PowerShell（`powershell.exe`）和命令提示符（`cmd.exe`）。该值是可执行文件名或路径，而不是 shell 命令行。现有窗格会保留当前 shell，直到重新创建。自定义命令按键绑定中的字符串在 Unix 上通过 `/bin/sh -c` 执行窗格命令，通过 `/bin/sh -lc` 执行分离式命令；在 Windows 上则通过 `cmd.exe /d /c` 执行。
...
在 `[ui]` 下设置 `language = "zh"`，即可使用内嵌的简体中文界面和 CLI 翻译。该设置在所有平台共用；Windows 的默认配置路径是 `%APPDATA%\herdr\config.toml`，Linux 和 macOS 则是 `~/.config/herdr/config.toml`。`zh-CN`、`zh-Hans` 等以 `zh` 开头的值也会启用中文；默认值 `en` 使用英文。

鼠标选择默认使用手动复制：拖选或双击终端文本后会保留选区，按 `Ctrl+C`、宿主转发的 `Cmd+C`、`Enter` 或 `y` 才复制，按 `Esc` 取消。设置 `copy_on_select = true` 或 `copy_on_select = "clipboard"` 可恢复选中后立即复制；手动模式也可显式写成 `false` 或 `"manual"`，`"disabled"` 则会完全禁用鼠标文本选择。
...
`row_gap` 分别控制 Agent 和 Space 面板的间距，不会在 `rows` 声明的内容行之间添加空白。Agent 条目把它作为条目间的空白行数。展开的每个 Space 条目（包括每个缩进的 worktree 子项）都有独立的交替背景和分隔线；Space 的 `row_gap` 会在这条必有的分隔线后增加空白行。两项设置的默认值都是 `0`。
