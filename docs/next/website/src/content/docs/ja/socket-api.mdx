...
| ポップアップ | `popup.close` |
| レイアウト | `layout.export`、`layout.apply`、`layout.set_split_ratio`、`layout.rearrange` |
...
`layout.rearrange` は、すでに実行中のペインの周囲にあるレイアウトツリーだけを再構築します。ペイン id、ターミナルの所有関係、プロセス状態、各ペインのスクロールバックは保持されます。再配置操作は `source_pane_id` を `target_pane_id` の隣へ移動します。`placement` は `left`、`right`、`up`、`down` のいずれかで、省略可能な `ratio` のデフォルトは `0.5` です。プリセット操作では `anchor_pane_id` が先頭またはメインになり、他のペインは現在の視覚順を保ちます。プリセットは `columns`、`rows`、`grid`、`main_left`、`main_top` です。
...
移動元と移動先は同じズームされていないタブに属している必要があります。レスポンスは `type: "layout_rearrange"` で、`rearrange` の下に `changed`、任意の `reason`、アンカー/対象/フォーカスペイン id、更新済みレイアウトスナップショットを含みます。変更されない場合の reason は `same_pane`、`cross_tab`、`zoomed_tab`、`single_pane`、`unchanged` です。変更に成功すると `layout.updated` が発行されます。
