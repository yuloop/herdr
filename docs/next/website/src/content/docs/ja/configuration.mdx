...
未設定または空の場合、Herdr は Unix では `$SHELL`、次に `/bin/sh` を使います。Windows では PowerShell 7（`pwsh.exe`）、Windows PowerShell（`powershell.exe`）、コマンド プロンプト（`cmd.exe`）の順に利用可能なシェルを選びます。これは実行ファイル名またはパスであり、シェルのコマンドラインではありません。既存のペインは再作成されるまで現在のシェルを保ちます。カスタムコマンドキーバインドの文字列は、Unix ではペインコマンドに `/bin/sh -c`、デタッチコマンドに `/bin/sh -lc` を通して実行され、Windows では `cmd.exe /d /c` を通して実行されます。
...
`row_gap` は Agent パネルと Space パネルの間隔を個別に制御し、`rows` で定義したコンテンツ行の間隔には影響しません。Agent ではエントリ間の空白行数です。展開された各 Space エントリ（インデントされた各 worktree 子も含む）には独立した交互背景と区切り線があり、Space の `row_gap` はその必須区切り線の後に空白行を追加します。どちらもデフォルトは `0` です。
