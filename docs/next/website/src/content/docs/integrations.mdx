---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, Qwen Code, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Qwen Code, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
The hook maps Qwen Code session, prompt, tool, permission, notification, compaction, stop, and release events to authoritative `working`, `blocked`, and `idle` state while Qwen runs inside a Herdr pane. When the hook is absent, Herdr falls back to the Qwen screen manifest.

Herdr uses `~/.qwen` by default, or `QWEN_HOME` when set. The Qwen config directory must already exist. Install writes `hooks/herdr-agent-state.sh` (or `.ps1` on Windows) and adds only Herdr-owned entries under the top-level `hooks` object in `settings.json`. Existing models, MCP servers, settings, and third-party hooks are preserved. Reinstall repairs Herdr entries without duplicating them; uninstall removes only those entries and the Herdr-owned script.

The hook records Qwen's session id as live pane identity and for native restore. Herdr resumes the exact saved conversation with `qwen --resume <id>`. The existing version 1 hook already reports the required id, so this support does not require reinstalling a current integration.
