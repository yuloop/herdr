...
Herdr is mouse-native, so start by clicking. Click panes, tabs, workspaces, and agents to focus them. Drag split borders to resize. Right-click for context menus, including splitting panes and creating tabs. Drag-select text or double-click a token, then press Enter or `y` to copy the retained selection; press Esc to cancel it. `Ctrl+C` also copies a retained selection.
