...
| Popup | `popup.close` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio`, `layout.rearrange` |
...
`layout.rearrange` rebuilds only the layout tree around panes that are already
running. It preserves pane ids, terminal ownership, process state, and every
pane's scrollback. A reposition operation moves `source_pane_id` beside
`target_pane_id`; `placement` is `left`, `right`, `up`, or `down`, and the
optional `ratio` defaults to `0.5`. A preset operation makes
`anchor_pane_id` the first/main pane and keeps the other panes in their current
visual order. Presets are `columns`, `rows`, `grid`, `main_left`, and
`main_top`.

```json
{"id":"req_reposition","method":"layout.rearrange","params":{"operation":{"type":"reposition","source_pane_id":"w1:p3","target_pane_id":"w1:p1","placement":"left"},"focus":true}}
{"id":"req_template","method":"layout.rearrange","params":{"operation":{"type":"preset","anchor_pane_id":"w1:p3","preset":"main_left"},"focus":true}}
```

Source and target must belong to the same unzoomed tab. The response is
`type: "layout_rearrange"` with `changed`, optional `reason`, anchor/target and
focused pane ids, and the updated layout snapshot under `rearrange`. No-op
reasons are `same_pane`, `cross_tab`, `zoomed_tab`, `single_pane`, and
`unchanged`. A successful mutation emits `layout.updated`.
