...
When unset or empty, Herdr uses `$SHELL`, then `/bin/sh` on Unix. On Windows, it tries PowerShell 7 (`pwsh.exe`), Windows PowerShell (`powershell.exe`), and Command Prompt (`cmd.exe`) in that order. This is an executable name or path, not a shell command line. Existing panes keep their current shell until they are recreated. Custom command keybinding strings run through `/bin/sh -c` for pane commands and `/bin/sh -lc` for detached commands on Unix; on Windows they run through `cmd.exe /d /c`.
...
Set `language = "zh"` under `[ui]` to use the embedded Simplified Chinese UI and CLI translations. The setting is shared by every platform; on Windows the default config path is `%APPDATA%\herdr\config.toml`, while Linux and macOS use `~/.config/herdr/config.toml`. Values beginning with `zh`, including `zh-CN` and `zh-Hans`, select Chinese; `en` is the default.

Mouse selection defaults to manual copying: drag-selecting or double-clicking terminal text retains the selection until `Ctrl+C`, a host-forwarded `Cmd+C`, `Enter`, or `y` copies it; `Esc` cancels it. Set `copy_on_select = true` or `copy_on_select = "clipboard"` to copy immediately after selecting. The equivalent manual values are `false` and `"manual"`; `"disabled"` turns mouse text selection off entirely.
...
`row_gap` controls spacing independently for the Agent and Space panels and does not add spacing between the content lines declared in `rows`. Agent entries use it as the number of blank rows between entries. Every expanded Space entry—including each indented worktree child—has its own alternating background and separator; the Space `row_gap` adds blank rows after that mandatory separator. Both settings default to `0`.
